...
const REQUIRED_RUNTIME_SOURCES = ["codex", "openclaw", "cloud-code"];
export async function validatePackage(packageDir, manifest) {
    ...
    const compatibility = await Promise.all((manifest.targets?.runtimes ?? []).map(async (runtime) => validateRuntime(packageDir, manifest, runtime)));
    return { issues, compatibility };
}
async function validateRuntime(packageDir, manifest, runtime) {
    const warnings = [];
    ...
    if (!validationSource) {
        blockingIssues.push(`Missing runtime validation source for '${runtime}'. Compatibility must be tied to docs or release tags.`);
    }
    ...
    if (!manifest.validation?.sources?.authoring_standards ||
        ...
        for (const [standardName, source] of Object.entries(manifest.validation.sources.authoring_standards)) {
            if (source.kind === "file") {
                const standardPath = path.join(packageDir, source.ref);
                if (!(await fileExists(standardPath))) {
                    warnings.push(`Authoring standard '${standardName}' references missing file '${source.ref}'.`);
                }
            }
        }
    }
...
function collectValidationSources(manifest, runtime) {
    const entries = [];
...
function inferVersionLabel(source) {
    if (!source) {
        return "unknown";
    }
    if (source.kind === "git") {
...
function deriveStatus(blockingIssues, warnings) {
